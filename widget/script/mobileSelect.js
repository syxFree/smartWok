...
(function () {
    ...
    MobileSelect.prototype = {
        ...
        init: function (config) {
            ...
            _this.checkDataType();
            _this.renderWheels(_this.wheelsData, config.cancelBtnText, config.ensureBtnText, config.isCanSwitchSelect);
        ...
        renderWheels: function (wheelsData, cancelBtnText, ensureBtnText, isCanSwitchSelect) {
            var _this = this;
            ...
            _this.mobileSelect.innerHTML =
                ...
                '</div>' +
                '</div>' +
                '<div id="mobileSelectContent"></div>' +
                '<div class="panel">' +
                '<div class="fixWidth">' +
                '<div class="wheels">' +
                '</div>' +
            ...
            if (isCanSwitchSelect) {
                var getCurValue = _this.getCurValue();
                var switchHtml =
                    '<div class="mobileSelectSwitchBtn"><div class="mobileSelectSwitchRules" onclick="switchSelectType()">按月选择 <img src="../../images/finance_switch.png" alt=""></div><div class="mobileSelectCheckAll" onclick="checkAllGoods()">查看全部</div></div>' +
                    '<div class="mobileSelectEcho"></div>';
                document.getElementById('mobileSelectContent').innerHTML = switchHtml;
            }
            //根据数据长度来渲染
        ...
        touch: function (event, theSlider, index) {
            ...
            switch (event.type) {
                ...
                case "touchend":
                    ...
                    var getCurValue = _this.getCurValue();
                    var mobileSelectEcho = '';
                    if (getCurValue.length == 2) {
                        mobileSelectEcho = getCurValue[0].value + '-' + getCurValue[1].value;
                    } else if (getCurValue.length == 1) {
                        mobileSelectEcho = getCurValue[0].value;
                    }
                    var result = mobileSelectEcho.replace(/(['一',('年月')])/gi, '');
                    if (document.getElementsByClassName('mobileSelectEcho')[0]) {
                        document.getElementsByClassName('mobileSelectEcho')[0].innerHTML = result;
                    }
                    break;
